...
export type LeaveRow = {
  ...
  type: string; range: string; days: number; halfDay: string | null;
  reason: string | null; status: Status; createdAt: string;
};
...
export function LeaveClient({ rows, canApprove }: { rows: LeaveRow[]; canApprove: boolean }) {
  ...
  return (
    <div className="page">
      ...
      {shown.length === 0 ? (
        <div className="req-empty">ยังไม่มีคำขอลาในหมวดนี้</div>
      ) : (
          ...
          {shown.map((r) => (
            <article key={r.id} className="req-card">
              ...
              <div className="req-meta">
                ...
                <span>{r.range}</span>
                <span className="req-dot">·</span>
                <span className="tabular">{r.days} วัน{r.halfDay ? ` (${r.halfDay})` : ""}</span>
              </div>
      ...
      {rejectId && (
        <div className="modal-scrim" onClick={() => setRejectId(null)}>
          <div className="modal" style={{ maxWidth: 440 }} onClick={(e) => e.stopPropagation()} role="alertdialog" aria-label="ปฏิเสธคำขอลา">
            <div className="modal-head"><span className="h">ปฏิเสธคำขอลา</span></div>
            <div className="modal-body">
              ...
              <textarea
                ...
                placeholder="เช่น ช่วงนี้มีงานด่วน ขอเลื่อนวันลา" value={reason}
