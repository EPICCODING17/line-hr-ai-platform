...
import { approveOtRequest, rejectOtRequest } from "./actions";

export type OtRow = {
  id: string; requestNo: string; employee: string; employeeCode: string;
  date: string; timeRange: string; hours: number; rate: string;
  project: string | null; reason: string | null; status: Status; createdAt: string;
};
...
export function OtClient({ rows, canApprove }: { rows: OtRow[]; canApprove: boolean }) {
  ...
  function doApprove(id: string) {
    setErr(null);
    start(async () => {
      const res = await approveOtRequest(id);
      setApproveId(null);
  ...
  function doReject(id: string) {
    setErr(null);
    start(async () => {
      const res = await rejectOtRequest(id, reason);
      setRejectId(null);
      setReason("");
      if (!res.ok) setErr(res.error);
      else router.refresh();
  ...
  return (
    <div className="page">
      ...
      {shown.length === 0 ? (
        <div className="req-empty">ยังไม่มีคำขอ OT ในหมวดนี้</div>
      ) : (
          ...
          {shown.map((r) => (
            <article key={r.id} className="req-card">
              ...
              <div className="req-meta">
                ...
                <span className="tabular">{r.timeRange}</span>
                <span className="req-dot">·</span>
                <span className="tabular">{fmtHours(r.hours)} ชม.</span>
                <span className="req-dot">·</span>
                <span>{r.rate}</span>
              </div>
              {r.project && <p className="req-reason">งาน: {r.project}</p>}
              {r.reason && <p className="req-reason">“{r.reason}”</p>}
      ...
      {rejectId && (
        <div className="modal-scrim" onClick={() => setRejectId(null)}>
          <div className="modal" style={{ maxWidth: 440 }} onClick={(e) => e.stopPropagation()} role="alertdialog" aria-label="ปฏิเสธคำขอ OT">
            <div className="modal-head"><span className="h">ปฏิเสธคำขอ OT</span></div>
            <div className="modal-body">
              ...
              <textarea
                ...
                placeholder="เช่น งานนี้ไม่ได้อนุมัติให้ทำ OT" value={reason}
